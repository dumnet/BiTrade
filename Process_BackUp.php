...
function Selling($order_spot, $spot, $order_id, $order_type, $order_value, $My_Btc, $My_Btc_to_Euro, $message, $date, $conn)
{
        ...
        if ($order_spot < $spot) {
            ...
            // Insert Transaction for Archive
            $query= "INSERT INTO transactions (id, id_ordre, type, value, spot, date_time, My_Btc, my_euro) VALUES (DEFAULT, '$order_id', '$order_type','$order_value', '$order_spot', '$date', '$My_Btc', '$My_Btc_to_Euro')";
            $result = $conn->query($query);
...
function Initialize($spot, $My_Btc, $My_Btc_to_Euro, $date, $conn)
{
        ...
        $query= "SELECT * FROM ordres WHERE 1";
...
function Process()
{
        ...
        while ($counter <= $while_times) {
            ...
            // Safe a Value if Diff not update
            if ($jump_spot == 0) {
